...
object RigBuilder {
	...
	private val bustWarpId = DeformerId("DeformBust")
	...
	private val armSwingLId = DeformerId("DeformArmSwingL")
	private val armSwingRId = DeformerId("DeformArmSwingR")
	private val armSwingContainerLId = DeformerId("DeformArmSwingContainerL")
	private val armSwingContainerRId = DeformerId("DeformArmSwingContainerR")
	...
	private data class DeformerBuildResult(
		...
		val armParentByLayerId: Map<String, Pair<DeformerId, Bounds>>,
	)
	...
	fun build(inputAnalysis: PipelineAnalysis, atlas: PackedAtlas, config: PipelineConfig): BuiltRig {
		...
		val eyeCloseLayers = layoutRigLayers.filter {
		...
		val deformerResult = if (shouldBuildDeformers) {
			...
			DeformerBuildResult(emptyList(), emptyMap(), emptyMap(), emptyMap())
		}
		...
		frameByDeformer[bustWarpId.raw] = characterFrame
		frameByDeformer[armSwingLId.raw] = characterFrame
		frameByDeformer[armSwingRId.raw] = characterFrame
		...
		for ((drawIndex, layer) in orderedLayers.withIndex()) {
			...
			val geometryGrid = if (config.meshOnly) {
				...
				buildDrawableGeometry(
					...
					matchingEyeCloseLayers(rigLayer, eyeCloseLayers),
					mouthAperture,
                    config,
				)
			}
			val override = config.layerOverrides[layer.source.id.raw]
			val channelGrids = if (config.meshOnly) ChannelGrids.Empty else buildChannels(layer, override, switchParamKeys, eyeCloseLayers.isNotEmpty())
	...
	/**
	 * Body Z leans the torso around its bottom-center pivot - Auto_Vtb's body lean (a rigid
	 * screen-plane rotation about the base, `body * 0.028` rad) - instead of the former per-row
	 * shear that read as a whole-body twist.  Breath (non-negative) lifts the chest and expands
	 * the ribcage symmetrically, applied first so the lean carries the breathing offset.
	 */
	internal fun bodySecondaryWarpPoint(
		...
		val cosA = cos(angle)
		val sinA = sin(angle)
		// Aspect-corrected so the lean is circular in canvas space, not elliptical in u/v.
		val aspect = character.height / character.width.coerceAtLeast(1e-4f)
		val invAspect = character.width / character.height.coerceAtLeast(1e-4f)
		val pv = 1f
		val dx = bx - pivotU
		val dy = by - pv
		val rx = pivotU + dx * cosA - dy * aspect * sinA
		val ry = pv + dx * invAspect * sinA + dy * cosA
		return rx to ry
	}

	/**
	 * Chest jiggle (Auto_Vtb's bust bounce): a vertical gaussian bump around the chest centre,
	 * driven by the physics-derived [bust] value (a transient spring, so it settles to zero).
	 * The gaussian keeps the displacement local to the torso; positive lifts, negative compresses.
	 */
	internal fun bustWarpPoint(
		u: Float,
		v: Float,
		chestU: Float,
		chestV: Float,
	...
	private fun buildDeformers(
		...
		val bodyPivotU = normalizeX(analysis.anchors.faceCenterX, character)
		...
		val hasTopwear = analysis.layers.any { it.semantic.tag == SemanticTag.TOPWEAR && it.opaquePixels > 0 }
		val bust = if (hasTopwear) {
			val faceAnchor = analysis.anchors.face
			val chestU = normalizeX(analysis.anchors.faceCenterX, character)
			val chestV = normalizeY(faceAnchor.bottom + 0.6f * faceAnchor.height + config.bustY * 0.10f * faceAnchor.height, character)
		...
		val deformers = mutableListOf<Deformer>(body, breath)
		if (bust != null) deformers += bust
		deformers += rotation
		...
		// Arm swing: each side gets one shoulder-pivot rotation deformer so the arm rotates around the
		// shoulder joint instead of the body deformer twisting the whole limb.  Like the head, the rotation
		// is wrapped by a pixel-space container warp: a rotation deformer's direct child cannot be a drawable
		// (drawables live in normalized [0,1] lattice space, not the rotation's local pixel space), so the
		// container is what actually parents the handwear drawables.  Driven by the ParamArmSwingL/R physics
		// pendulums, which the software fallback mirrors with a spring.
		val armParentByLayerId = mutableMapOf<String, Pair<DeformerId, Bounds>>()
		val armLayers = analysis.layers.filter {
			it.semantic.tag == SemanticTag.HANDWEAR && it.opaquePixels > 0 && it.semantic.side != Side.NONE
		}
		for (side in listOf(Side.LEFT, Side.RIGHT)) {
			val sideLayers = armLayers.filter { it.semantic.side == side }
			if (sideLayers.isEmpty()) continue
			val isLeft = side == Side.LEFT
			val deformerId = if (isLeft) armSwingLId else armSwingRId
			val containerId = if (isLeft) armSwingContainerLId else armSwingContainerRId
			val parameter = if (isLeft) StandardParameters.ARM_SWING_L else StandardParameters.ARM_SWING_R
			val armBounds = sideLayers.map { rigLayerById.getValue(it.source.id.raw).bounds }.reduce(Bounds::union)
			// The shoulder pivot sits on the arm's top edge at the body-facing corner: for the character's
			// left arm (viewer's right) that is the arm's left edge, for the right arm its right edge.  Using
			// the arm's own horizontal center pivots the limb mid-air and tears it off the torso when it swings.
			val pivotCanvasX = if (isLeft) armBounds.left else armBounds.right
			val pivotCanvasY = armBounds.top
			val pivotX = normalizeX(pivotCanvasX, character)
			val pivotY = normalizeY(pivotCanvasY, character)
			val swingGrid = oneDimGrid(parameter, floatArrayOf(-30f, 0f, 30f)) { value ->
				RotationPivotForm(pivotX, pivotY, value, 1f)
			}
			deformers += Deformer.Rotation(
				deformerId,
				tr("model.deformer.armSwing"),
				breathWarpId,
				bodyPartId,
				0f,
				swingGrid,
			...
			val containerGrid = warpGrid(emptyList(), columns = 3, rows = 3) { u, v, _ ->
				(armBounds.left + u * armBounds.width - pivotCanvasX) to (armBounds.top + v * armBounds.height - pivotCanvasY)
			}
			deformers += Deformer.Warp(
				containerId,
				tr("model.deformer.armSwingContainer"),
				deformerId,
				bodyPartId,
				3,
				3,
			...
			for (layer in sideLayers) {
				armParentByLayerId[layer.source.id.raw] = containerId to armBounds
			}
		}
		...
		val candidateLayers = analysis.layers.filter { layer ->
			layer.opaquePixels > 0 &&
				...
				layer.semantic.tag != SemanticTag.HANDWEAR &&
		...
		return DeformerBuildResult(deformers, pairFrames, pairedParentByLayerId, armParentByLayerId)
	}
	...
	private fun buildGridMesh(
		...
		if (adaptive != null) {
			...
			for (index in adaptive.positions.indices step 2) {
				...
				var canvasX = layer.source.bounds.left + localX
				var canvasY = layer.source.bounds.top + localY
				if (layer.semantic.tag == SemanticTag.HANDWEAR) {
					val armScale = parentFrame.height
					val sideSign = when (layer.semantic.side) {
						Side.LEFT -> -1f
						Side.RIGHT -> 1f
						else -> 0f
					}
					canvasX += config.armY * 0.006f * armScale * sideSign
					canvasY += (-config.armY * 0.030f + config.armPos * 0.040f) * armScale
				}
	...
	private fun buildDrawableGeometry(
		...
		eyeCloseLayers: List<ClassifiedLayer>,
		...
		return when (tag) {
			SemanticTag.EYEWHITE, SemanticTag.EYELASH ->
				eyeClosureGrid(layer, data, parentFrame, faceRig, eyeWhiteBounds, eyeCloseLayers)
	...
	private fun eyeClosureGrid(
		...
		return grid(axes) { values ->
			...
			for (vertex in data.mesh.positions.indices step 2) {
				...
				// Closed-eye seam line: the eye_close centroid, else 62% down the eye white (rigger.js).
				val closeY = eyeCloseLayers.minByOrNull { abs(it.centroidX - canvasX) }?.centroidY
					?: (whiteBounds.top + whiteBounds.height * 0.62f)
				val closedY = eyeClosurePoint(closeY, canvasY)
				delta[vertex + 1] = (closedY - canvasY) / frame.height.coerceAtLeast(1e-4f) * (1f - openness)
			}
			MeshDeltaForm(delta)
		}
	}
	...
	/**
	 * Closed-eyelid target mirroring rigger.js lid compression: every vertex collapses toward the
	 * closed-eye seam line (closeY) with X held fixed, leaving 15% of its distance at full closure.
	 * The keyform grid interpolates linearly with openness, which reproduces rigger.js exactly:
	 * `y = closeY + (y - closeY) * (1 - 0.85 * (1 - vOpen))`.  A full lash ring folds onto the line
	 * as one closed lid; a per-column arch or a hairline collapse reads wrong (see EyeClosureTest).
	 */
	internal fun eyeClosurePoint(closeY: Float, sourceY: Float): Float =
		closeY + (sourceY - closeY) * 0.15f

	internal fun eyeClosurePoint(eyeWhiteBounds: Bounds, sourceY: Float): Float =
		eyeClosurePoint(eyeWhiteBounds.top + eyeWhiteBounds.height * 0.62f, sourceY)
	...
	private fun matchingEyeCloseLayers(
		layer: ClassifiedLayer,
		eyeCloses: List<ClassifiedLayer>,
	): List<ClassifiedLayer> {
		if (eyeCloses.isEmpty()) return emptyList()
		val sameVariantAndSide = eyeCloses.filter {
			it.semantic.side == layer.semantic.side && it.semantic.variant == layer.semantic.variant
		}
		val sameSide = eyeCloses.filter { it.semantic.side == layer.semantic.side }
		val unspecified = eyeCloses.filter { it.semantic.side == Side.NONE }
		val candidates = when {
			sameVariantAndSide.isNotEmpty() -> sameVariantAndSide
			sameSide.isNotEmpty() -> sameSide
			layer.semantic.side == Side.NONE -> eyeCloses
			unspecified.isNotEmpty() -> unspecified
			else -> eyeCloses
		}
		if (layer.semantic.side == Side.NONE && candidates.size > 1) return candidates
		return listOfNotNull(nearestLayer(layer, candidates))
	}
		...
		val opacityGrid = when (type) {
			...
			LayerType.PRESET -> {
				when (layer.semantic.tag) {
					// rigger.js fadeAlpha 'eyeOpen': open-eye parts fade out as the lid closes.
					SemanticTag.EYEWHITE, SemanticTag.IRIDES -> {
						val parameter = if (layer.semantic.side == Side.LEFT) StandardParameters.EYE_L_OPEN else StandardParameters.EYE_R_OPEN
						scalarGrid(parameter, floatArrayOf(0f, 0.10f, 0.175f, 0.25f, 1f)) { open ->
							val t = ((open - 0.10f) / 0.15f).coerceIn(0f, 1f)
							t * t * (3f - 2f * t)
						}
					}
					// The eyelash is a full lash ring that itself becomes the closed lid when it is
					// squashed onto closeY, so it only fades out when a separate eye_close drawing takes
					// over the closed-eye appearance (rigger.js synthesizes one when it is absent).
					SemanticTag.EYELASH -> if (hasEyeClose) {
						val parameter = if (layer.semantic.side == Side.LEFT) StandardParameters.EYE_L_OPEN else StandardParameters.EYE_R_OPEN
						scalarGrid(parameter, floatArrayOf(0f, 0.10f, 0.175f, 0.25f, 1f)) { open ->
							val t = ((open - 0.10f) / 0.15f).coerceIn(0f, 1f)
							t * t * (3f - 2f * t)
