...
internal object Cmo3PhysicsInjector {
	fun inject(root: CModelSource, hasFrontHair: Boolean, hasBackHair: Boolean, hasEyeJelly: Boolean = false, custom: List<RigPhysicsEdit> = emptyList(), hasBust: Boolean = false, hasArms: Boolean = false, tuning: PhysicsTuning = PhysicsTuning()): Int {
		...
		val rules = PhysicsGenerator.mergeCustomRules(PhysicsGenerator.validRules(hasFrontHair, hasBackHair, hasEyeJelly, parameterById.keys, hasBust, hasArms, tuning), custom, parameterById.keys)
