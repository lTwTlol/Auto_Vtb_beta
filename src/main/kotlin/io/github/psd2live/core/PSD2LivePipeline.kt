...
class PSD2LivePipeline {
	...
	private fun exportAnalysis(
		...
		val hasBust = analysis.layers.any { it.semantic.tag == SemanticTag.TOPWEAR && it.opaquePixels > 0 }
		val hasArms = analysis.layers.any { it.semantic.tag == SemanticTag.HANDWEAR && it.opaquePixels > 0 }
	...
	private fun projectReport(
		...
		val useBust = hasBust && config.generatePhysics && !config.meshOnly
		return """
		{
		  "version": 1,
		  ...
		  "physics": {"enabled":${config.generatePhysics && !config.meshOnly},"frontHair":$useFrontHair,"backHair":$useBackHair,"eyeJelly":$useEyeJelly,"bust":$useBust,"preset":"hair-and-eye-pendulum"},
