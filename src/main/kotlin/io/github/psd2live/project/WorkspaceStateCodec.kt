...
internal object WorkspaceStateCodec {
    ...
    fun settings(state: PSD2LiveState): JsonObject = buildJsonObject {
        ...
        put("bustAmp", state.bustAmp)
        put("bustY", state.bustY)
        put("armY", state.armY)
        put("armPos", state.armPos)
        put("frontHairAmp", state.frontHairAmp)
        put("frontHairSoft", state.frontHairSoft)
        put("backHairAmp", state.backHairAmp)
        put("backHairSoft", state.backHairSoft)
        put("armSwingAmp", state.armSwingAmp)
        put("faceBodyFollow", state.faceBodyFollow)
    ...
    fun encode(state: PSD2LiveState): JsonObject = buildJsonObject {
        ...
        put("activeExpressionPreset", state.activeExpressionPreset?.let(::JsonPrimitive) ?: JsonNull)
        put("previewBackground", state.previewBackground.name)
    ...
    fun decode(value: JsonObject, base: PSD2LiveState = PSD2LiveState()): PSD2LiveState = base.copy(
        ...
        bustAmp = value["bustAmp"]?.jsonPrimitive?.float ?: base.bustAmp,
        bustY = value["bustY"]?.jsonPrimitive?.float ?: base.bustY,
        armY = value["armY"]?.jsonPrimitive?.float ?: base.armY,
        armPos = value["armPos"]?.jsonPrimitive?.float ?: base.armPos,
        frontHairAmp = value["frontHairAmp"]?.jsonPrimitive?.float ?: base.frontHairAmp,
        frontHairSoft = value["frontHairSoft"]?.jsonPrimitive?.float ?: base.frontHairSoft,
        backHairAmp = value["backHairAmp"]?.jsonPrimitive?.float ?: base.backHairAmp,
        backHairSoft = value["backHairSoft"]?.jsonPrimitive?.float ?: base.backHairSoft,
        armSwingAmp = value["armSwingAmp"]?.jsonPrimitive?.float ?: base.armSwingAmp,
        faceBodyFollow = value["faceBodyFollow"]?.jsonPrimitive?.float ?: base.faceBodyFollow,
        ...
        activeExpressionPreset = if ("activeExpressionPreset" in value) value["activeExpressionPreset"]?.jsonPrimitive?.contentOrNull else base.activeExpressionPreset,
        previewBackground = value["previewBackground"]?.jsonPrimitive?.content?.let { PreviewBackground.valueOf(it) } ?: base.previewBackground,
